...
(function($){
    ...
    $.parentReload = function (moudle) {
        ...
        if(hasParent && moudle){
            ...
            //ajax刷新父级表格
            else if(moudle == 'table'){
                if(!parent.vue1 || !parent.vue1.table1 || !parent.vue1.table1.reload){
                    parent.layer.alert('父窗口方法未定义：vue1.table1.reload()');
                    return;
                }
    ...
    $.ajax=function(opt) {
        ...
        opt.errorMsg = opt.errorMsg || false;
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                try {
                    ...
                    if(!XMLHttpRequest.readyState){
                        msg = (typeof(LANAGE) != 'undefined' ? LANAGE.NO_NETWORK : '未连接到互联网，请检查网络连接');
                    }
                    console.error(msg);
                    if (opt.errorMsg !== false) {
                        var config = {icon: 5, btn: (typeof(LANAGE) != 'undefined' ? LANAGE.BUTTON_OK : undefined)};
                        layer.alert(msg, config, function (index) {
